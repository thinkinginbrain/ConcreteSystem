...
import com.wzc.login.dao.LimitDao;
...
public class LimitServlet extends HttpServlet {
	...
	private LimitDao dao = new LimitDao();
	...
	private void toUpdatePage(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException  {
		...
		Limit user = dao.selectByUserId(id);
		request.setAttribute("updateUser", user);
		request.getRequestDispatcher("pages/usermgr/limit_update.jsp").forward(request, response);
	}

	private void selectAllUsers(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		List<Limit> list = dao.selectByMap(new HashMap<String,Object>(0));
		request.setAttribute("userList", list);
		request.getRequestDispatcher("pages/usermgr/limit.jsp").forward(request, response);
	}
...
//	}
	...
	private void updateUser(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if(dao.update(user)){
			...
			response.sendRedirect("LimitServlet?act=select");
		}else {
			request.setAttribute("updateUser", user);
