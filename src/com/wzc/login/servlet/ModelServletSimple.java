...
import com.wzc.login.dao.ModelDao;
 ...
 */
@WebServlet("/ModelServletSimple")
public class ModelServletSimple extends HttpServlet {
	...
	private void toUpdatePage(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException  {
		...
		Model user = dao.selectByUserId(userid);
		request.setAttribute("updateUser", user);
		request.getRequestDispatcher("pages/usermgr/model_update.jsp").forward(request, response);
	}
//
	private void selectAllUsers(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		List<Model> list = dao.selectByMap(new HashMap<String,Object>(0));
		request.setAttribute("modelList", list);
		request.getRequestDispatcher("pages/usermgr/ModelMessageSimple.jsp").forward(request, response);
	}
	...
	private void insertUser(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if(dao.insert(user)){
			...
			response.sendRedirect("ModelServlet?act=select");
		}else {
			request.getSession().setAttribute("resultMSG", "新增失败");
			request.getRequestDispatcher("pages/usermgr/insert_model.jsp").forward(request, response);
		}
	}
	...
	private void updateUser(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		Model user = new Model();
		if(userid!=null){
			user.setModelid(Integer.valueOf(userid));
		}
		if(name!=null){
			user.setName(name);
		}
